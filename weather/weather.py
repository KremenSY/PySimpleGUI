...
while True:
	...
	if event == 'submit':
		...
		if weather in ('Sun','Sunny','Clear','Clear with periodic clouds', 'Mostly sunny'):
			window['-IMAGE-'].update(os.path.join(os.path.dirname(__file__), 'sun.png'))
		
		# part sun
		if weather in ('Partly Sunny','Mostly Sunny','Partly cloudy','Mostly cloudy','Cloudy','Overcast'):
			window['-IMAGE-'].update(os.path.join(os.path.dirname(__file__),'part_sun.png'))
		
		# rain
		if weather in ('Rain','Chance of Rain','Light Rain','Showers','Scattered Showers','Rain and Snow','Hail'):
			window['-IMAGE-'].update(os.path.join(os.path.dirname(__file__), 'rain.png'))
		
		# thunder
		if weather in ('Scattered Thunderstorms','Chance of Storm','Storm','Thunderstorm','Chance of TStorm'):
			window['-IMAGE-'].update(os.path.join(os.path.dirname(__file__),'thunder.png'))

		# foggy
		if weather in ('Mist','Dust','Fog','Smoke','Haze','Flurries'):
			window['-IMAGE-'].update(os.path.join(os.path.dirname(__file__),'fog.png'))
		
		# snow
		if weather in ('Freezing Drizzle','Chance of Snow','Sleet','Snow','Icy','Snow Showers'):
			window['-IMAGE-'].update(os.path.join(os.path.dirname(__file__),'snow.png'))
