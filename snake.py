...
def convert_pos_to_pixel(cell):
    ...
    br=(cell[0]+1)*CELL_SIZE, (cell[1]+1)*CELL_SIZE
    return tl, br

def place_apple():
    apple_pos=(randint(0, CELL_NUM-1), randint(0, CELL_NUM-1))
    while apple_pos in snake_body:
        apple_pos=(0, randint(CELL_NUM-1), randint(0, CELL_NUM-1))
    return apple_pos
...
window = sg.Window('CWindow name', layout, return_keyboard_events=True)
